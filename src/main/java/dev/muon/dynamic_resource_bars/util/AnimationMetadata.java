...
public class AnimationMetadata {
    ...
    public static class AnimationData {
        ...
        public final boolean vanillatiling;
        public final int textureWidth;
        public final int textureHeight;

        public AnimationData(int frametime, int frameHeight, int totalFrames, boolean interpolate,
            boolean vanillatiling, int textureWidth, int textureHeight) {
            this.frametime = frametime;
            this.frameHeight = frameHeight;
            this.totalFrames = totalFrames;
            this.interpolate = interpolate;
            this.vanillatiling = vanillatiling;
     ...
     * Allows nine-slicing a specific area (e.g., 80x10 in top-left of 256x256
     * sheet).
     */
    ...
    public static AnimationData loadAnimationData(ResourceManager resourceManager, 
        ...
        final boolean DEFAULT_INTERPOLATE = true;
        final boolean DEFAULT_VANILLA_TILING = false;
        ...
        try {
            ...
            if (resourceOpt.isEmpty()) {
                ...
                return new AnimationData(DEFAULT_FRAMETIME, DEFAULT_FRAME_HEIGHT, DEFAULT_TOTAL_FRAMES,
                    DEFAULT_INTERPOLATE, DEFAULT_VANILLA_TILING,
            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                boolean interpolate = animation.has("interpolate")
                    ...
                    : DEFAULT_INTERPOLATE;
                boolean vanillatiling = animation.has("vanillatiling")
                    ? animation.get("vanillatiling").getAsBoolean()
                    : DEFAULT_VANILLA_TILING;
                ...
                DynamicResourceBars.LOGGER.info(
                    "ANIM_LOAD: Successfully loaded animation data for {}: frametime={}, frameHeight={}, totalFrames={}, textureSize={}x{}, interpolate={}, vanillatiling={}",
                    textureLocation, frametime, frameHeight, totalFrames, textureDims.width, textureDims.height,
                    interpolate, vanillatiling);
                
                return new AnimationData(frametime, frameHeight, totalFrames, interpolate, vanillatiling,
                    textureDims.width, textureDims.height);
    ...
    /**
     * Load actual texture dimensions from PNG file (public version for non-animated
     * textures).
     * This allows resource packs to use any texture size!
     */
    public static TextureDimensions loadTextureDimensions(ResourceManager resourceManager,
            ResourceLocation textureLocation) {
        // Defaults if we can't load the texture
        final int DEFAULT_WIDTH = 256;
        final int DEFAULT_HEIGHT = 1024;

        DynamicResourceBars.LOGGER.info("DIMENSION_LOAD: Attempting to load texture dimensions for: {}",
                textureLocation);
        ...
        try {
            ...
            DynamicResourceBars.LOGGER.info("DIMENSION_LOAD: Resource found for {}, attempting to read PNG...",
                    textureLocation);
            ...
            try (InputStream stream = resourceOpt.get().open()) {
                ...
                DynamicResourceBars.LOGGER.info(
                        "DIMENSION_LOAD: Successfully detected texture dimensions for {}: {}x{}",
                        textureLocation, width, height);
                return new TextureDimensions(width, height);
            }
        } catch (Exception e) {
            DynamicResourceBars.LOGGER.info(
                    "DIMENSION_LOAD: ERROR loading texture dimensions for {}: {} ({}). Using defaults {}x{}",
    ...
    /**
     * Calculate the animation offset (V coordinate in texture) for the current
     * frame
     * 
     ...
     * Note: BAR type exists but should not use nine-slice (uses UV sampling
     * instead).
     */
    ...
    private static ScalingInfo getDefaultScaling(TextureType type) {
        switch (type) {
            case BAR:
                ...
                // If scaling is requested for a bar (shouldn't happen), log warning and return
                // stretch
                DynamicResourceBars.LOGGER
                        .warn("Scaling requested for BAR texture - bars should use UV sampling, not scaling!");
