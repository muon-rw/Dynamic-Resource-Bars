...
public class AnimationMetadata {
    ...
    public static AnimationData loadAnimationData(ResourceManager resourceManager, 
        ...
        Constants.LOG.info("ANIM_LOAD: Starting animation data load for: {}", textureLocation);
        
        // Create .mcmeta location
        Identifier mcmetaLocation;
        mcmetaLocation = Identifier.fromNamespaceAndPath(
            textureLocation.getNamespace(),
            textureLocation.getPath() + ".mcmeta"
        );
        
        Constants.LOG.info("ANIM_LOAD: Looking for .mcmeta at: {}", mcmetaLocation);
        ...
        Constants.LOG.info("ANIM_LOAD: Texture dimensions loaded: {}x{}", textureDims.width, textureDims.height);
        
        try {
            Optional<Resource> resourceOpt = resourceManager.getResource(mcmetaLocation);
            if (resourceOpt.isEmpty()) {
                // No .mcmeta file found, use hardcoded defaults
                Constants.LOG.info("ANIM_LOAD: No .mcmeta found for {}, using hardcoded defaults (frametime={}, frameHeight={}, totalFrames={})", 
            ...
            Constants.LOG.info("ANIM_LOAD: .mcmeta resource found, parsing JSON...");
            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                Constants.LOG.info("ANIM_LOAD: JSON parsed successfully, checking for 'animation' section...");
                
                if (!root.has("animation")) {
                    Constants.LOG.info("ANIM_LOAD: INVALID .mcmeta for {}: Missing 'animation' section! Using defaults.", textureLocation);
                    return new AnimationData(DEFAULT_FRAMETIME, DEFAULT_FRAME_HEIGHT, DEFAULT_TOTAL_FRAMES, DEFAULT_INTERPOLATE,
                        textureDims.width, textureDims.height);
                }
                
                JsonObject animation = root.getAsJsonObject("animation");
                Constants.LOG.info("ANIM_LOAD: Found 'animation' section");
                ...
                if (animation.has("height")) {
                    ...
                    Constants.LOG.info("ANIM_LOAD: Frame height from 'height' field: {}", frameHeight);
                } else if (animation.has("frameHeight")) {
                    frameHeight = animation.get("frameHeight").getAsInt();
                    Constants.LOG.info("ANIM_LOAD: Frame height from 'frameHeight' field: {}", frameHeight);
                } else {
                    Constants.LOG.info("ANIM_LOAD: INVALID .mcmeta for {}: Missing 'height' field! Using default ({}px).", 
                ...
                Constants.LOG.info("ANIM_LOAD: Calculated totalFrames = {} / {} = {}", 
                    textureDims.height, frameHeight, totalFrames);
                
                Constants.LOG.info("ANIM_LOAD: Successfully loaded animation data for {}: frametime={}, frameHeight={}, totalFrames={}, textureSize={}x{}, interpolate={}", 
                ...
                Constants.LOG.info("ANIM_LOAD: ERROR parsing .mcmeta for {}: {} ({}). Using hardcoded defaults.", 
            ...
            Constants.LOG.info("ANIM_LOAD: ERROR loading .mcmeta for {}: {} ({}). Using hardcoded defaults.", 
                textureLocation, e.getClass().getSimpleName(), e.getMessage());
            e.printStackTrace();
            return new AnimationData(DEFAULT_FRAMETIME, DEFAULT_FRAME_HEIGHT, DEFAULT_TOTAL_FRAMES, DEFAULT_INTERPOLATE,
                textureDims.width, textureDims.height);
     ...
     */
    public static TextureDimensions loadTextureDimensions(ResourceManager resourceManager, Identifier textureLocation) {
        // Defaults if we can't load the texture
        final int DEFAULT_WIDTH = 256;
        final int DEFAULT_HEIGHT = 1024;
        
        Constants.LOG.info("DIMENSION_LOAD: Attempting to load texture dimensions for: {}", textureLocation);
        
        try {
            Optional<Resource> resourceOpt = resourceManager.getResource(textureLocation);
            if (resourceOpt.isEmpty()) {
                Constants.LOG.info("DIMENSION_LOAD: Texture not found: {}. Using default dimensions {}x{}", 
                    textureLocation, DEFAULT_WIDTH, DEFAULT_HEIGHT);
                return new TextureDimensions(DEFAULT_WIDTH, DEFAULT_HEIGHT);
            }
            
            Constants.LOG.info("DIMENSION_LOAD: Resource found for {}, attempting to read PNG...", textureLocation);
            ...
            try (InputStream stream = resourceOpt.get().open()) {
                ...
                Constants.LOG.info("DIMENSION_LOAD: Successfully detected texture dimensions for {}: {}x{}", 
                    textureLocation, width, height);
                return new TextureDimensions(width, height);
            }
        } catch (Exception e) {
            Constants.LOG.info("DIMENSION_LOAD: ERROR loading texture dimensions for {}: {} ({}). Using defaults {}x{}", 
     ...
     */
    public static BarDimensions loadBarDimensions(ResourceManager resourceManager, Identifier textureLocation) {
        Identifier mcmetaLocation;
        mcmetaLocation = Identifier.fromNamespaceAndPath(
            textureLocation.getNamespace(),
            textureLocation.getPath() + ".mcmeta"
        ...
        try {
            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                if (!bar.has("width") || !bar.has("height")) {
                    Constants.LOG.error("INVALID bar dimensions in .mcmeta for {}: missing width or height", textureLocation);
                    return null;
                }
                ...
                Constants.LOG.info("Loaded bar dimensions from .mcmeta for {}: {}x{}", textureLocation, width, height);
                return new BarDimensions(width, height, true);
                
            } catch (Exception e) {
                Constants.LOG.error("ERROR parsing bar dimensions for {}: {}", textureLocation, e.getMessage());
                return null;
            }
        } catch (Exception e) {
     ...
     */
    public static ScalingInfo loadScalingInfo(ResourceManager resourceManager, Identifier textureLocation, TextureType textureType) {
        Identifier mcmetaLocation;
        mcmetaLocation = Identifier.fromNamespaceAndPath(
            textureLocation.getNamespace(),
            textureLocation.getPath() + ".mcmeta"
            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                if (custom.get("scaling").isJsonPrimitive()) {
                    ...
                    switch (scalingStr) {
                        ...
                        default:
                            Constants.LOG.warn("Unknown scaling mode '{}' for {}. Using default.", scalingStr, textureLocation);
                            return getDefaultScaling(textureType);
                    }
                    ...
                    if (!scaling.has("type") || !scaling.get("type").getAsString().equals("nine_slice")) {
                        Constants.LOG.error("INVALID scaling for {}: nine-slice object must have type='nine_slice'", textureLocation);
                        return getDefaultScaling(textureType);
                    }
                    
                    if (!scaling.has("left") || !scaling.has("right") || !scaling.has("top") || !scaling.has("bottom")) {
                        Constants.LOG.error("INVALID nine-slice for {}: missing border dimensions", textureLocation);
                        return getDefaultScaling(textureType);
                    }
                    ...
                    Constants.LOG.info("Loaded nine-slice scaling for {}: borders=({},{},{},{}), edges={}, center={}", 
                ...
                Constants.LOG.warn("Invalid scaling format for {}. Using default.", textureLocation);
                return getDefaultScaling(textureType);
                
            } catch (Exception e) {
                Constants.LOG.error("ERROR parsing scaling info for {}: {}", textureLocation, e.getMessage());
                return getDefaultScaling(textureType);
            }
        } catch (Exception e) {
            return getDefaultScaling(textureType);
    ...
    private static ScalingInfo getDefaultScaling(TextureType type) {
        switch (type) {
            case BAR:
                ...
                Constants.LOG.warn("Scaling requested for BAR texture - bars should use UV sampling, not scaling!");
     ...
     */
    private static SourceRegion parseSourceRegion(JsonObject source, Identifier textureLocation) {
        if (source.has("width") && source.has("height")) {
            ...
            Constants.LOG.info("Loaded source region for {}: u={}, v={}, {}x{}", 
