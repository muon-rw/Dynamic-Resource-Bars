...
public class TextureMetadata {
     ...
     */
    public static FullMetadata loadFullMetadata(ResourceManager resourceManager, Identifier textureLocation) {
        ...
        try {
            ...
            if (texResourceOpt.isPresent()) {
                try (InputStream stream = texResourceOpt.get().open()) {
                    ...
                    image.close();
                    Constants.LOG.info("Detected texture size for {}: {}x{}", textureLocation, textureWidth, textureHeight);
                }
            }
        } catch (Exception e) {
            Constants.LOG.warn("Could not read texture dimensions for {}, using defaults", textureLocation);
        }
        ...
        try {
            ...
            if (resourceOpt.isPresent()) {
                ...
                try (BufferedReader reader = new BufferedReader(
                    ...
                    Constants.LOG.error("ERROR parsing .mcmeta for {}: {}", textureLocation, e.getMessage());
                }
            }
        } catch (Exception e) {
            Constants.LOG.info("No .mcmeta for {} or could not load", textureLocation);
        }
        ...
        if (animation == null) {
            ...
            Constants.LOG.warn("No animation data in .mcmeta for {}, using defaults (frametime=3, height=32)", textureLocation);
        }
    ...
    private static AnimationInfo parseAnimation(JsonObject root, Identifier location, int textureHeight) {
        ...
        if (animation.has("height")) {
            ...
            Constants.LOG.error("INVALID .mcmeta for {}: Missing 'height' field in animation! Using 32px.", location);
            frameHeight = 32;
        }
        
        int totalFrames = textureHeight / frameHeight;
        
        Constants.LOG.info("Loaded animation for {}: frametime={}, frameHeight={}, totalFrames={}", 
    ...
    private static UVMapping parseUVMapping(JsonObject uv, int textureWidth, int defaultHeight) {
        ...
        Constants.LOG.info("Loaded UV mapping: u={}, v={}, width={}, height={}", u, v, width, height);
    ...
    private static NineSliceInfo parseNineSlice(JsonObject nineSlice) {
        ...
        Constants.LOG.info("Loaded nine-slice: left={}, right={}, top={}, bottom={}", left, right, top, bottom);
